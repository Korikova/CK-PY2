...
class Boat:
    ...
    def ready_to_depart(self) -> bool:
        ...
        """
        if self.max_capacity_volume == self.occupied_volume:
            return True
        else:
            return False
...
class WashingMachine:
    ...
    def checking_for_width(self) -> bool:
        ...
        """
        if self.width_wm <= self.width_space:
            return True
        else:
            return False
    ...
    def checking_for_length(self) -> bool:
        ...
        """
        if self.length_wm <= self.length_space:
            return True
        else:
            return False
...
class License:
    ...
    def can_drive(self) -> bool:
        ...
        """
        if self.age >= self.acceptable_age:
            return True
        else:
            return False
    ...
    def time(self):
        ...
        """
        ost = self.acceptable_age - self.age
        if ost < 0 or ost == 0:
            raise ValueError("Вы достигли приемлемого возраста для обучения на права")
        if ost > 0:
            return ost


if __name__ == "__main__":
    boat = Boat(100.0, 90.0)
    boat.ready_to_depart()
    boat.add_cargo_to_boat(30.0)
    washing_machine = WashingMachine(9.0, 10.0, 8.0, 12.0)
    washing_machine.checking_for_width()
    washing_machine.checking_for_length()
    license_ = License("Иванов", 14, 16)
    license_.can_drive()
    license_.time()
    doctest.testmod()
    pass
